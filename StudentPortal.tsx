            ...
            {STUDENT_GRADES.map((grade, idx) => (
              <tr key={idx} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${grade.grade === 'A' ? 'bg-green-100 text-green-800' :
            ...
            <Card key={item.id} className="p-5 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded uppercase ${item.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {STUDENT_FINANCE.map((record, idx) => (
                <tr key={idx}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${record.status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
